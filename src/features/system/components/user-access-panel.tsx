 ...
 * The server refuses changes to your own account and removing the last active admin;
        ...
        <ul className="mt-2 space-y-1">
          ...
          {account.roles.map((role) => (
            <li key={role.roleId} className="flex items-center justify-between gap-2 rounded-xl border border-border bg-field px-3 py-2 text-xs">
              ...
              {!isSelf && <button type="button" disabled={busy} onClick={() => revokeRole(role.roleId)} className="flex items-center gap-1 text-[#8e1f33] hover:underline disabled:opacity-50"><ShieldOff className="size-3.5" />Quitar</button>}
            </li>
          ))}
        </ul>
        {!isSelf && (
        ...
        <ul className="mt-2 space-y-1">
          ...
          {account.grants.map((item) => (
            <li key={item.id} className="flex items-center justify-between gap-2 rounded-xl border border-border bg-field px-3 py-2 text-xs">
              ...
              {!isSelf && <button type="button" disabled={busy} onClick={() => revokeGrant(item.id)} className="flex items-center gap-1 text-[#8e1f33] hover:underline disabled:opacity-50"><ShieldOff className="size-3.5" />Quitar</button>}
            </li>
          ))}
        </ul>
        {!isSelf && (
          <div className="mt-2 grid gap-2 md:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)_auto]">
            <select value={draftPermission} onChange={(event) => setDraftPermission(event.target.value)} className={input}>
              ...
              {permissions.filter((item) => item.grantable).map((item) => <option key={item.code} value={item.code}>{item.description}</option>)}
            </select>
