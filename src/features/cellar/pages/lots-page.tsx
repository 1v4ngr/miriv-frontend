...
function LotForm({ deposits, existingLots, defaultResponsible, defaultResponsibleUsername, onClose, onCreated }: { deposits: Deposit[]; existingLots: Lot[]; defaultResponsible: string; defaultResponsibleUsername: string; onClose: () => void; onCreated: (lot: Lot) => void }) {
  const initialCategory = ''
  const initialCampaign = new Date().getFullYear()
  const initialResponsible = defaultResponsibleUsername || defaultResponsible
  ...
  useEffect(() => {
    ...
    profileApi.listCenterMembers().then((items) => {
      ...
      setLot((current) => {
        if (current.responsible && items.some((member) => member.username === current.responsible || member.displayName === current.responsible)) return current
        const fallback = items.find((member) => member.username === defaultResponsibleUsername) ?? items.find((member) => member.displayName === defaultResponsible)
        return fallback ? { ...current, responsible: fallback.username } : current
      })
    }).catch(() => undefined)
  }, [])

  // Auto-fill the code whenever category or campaign changes — unless the user has typed their own.
  useEffect(() => {
...
export function LotsPage({ search, onOpenDeposit, onOpenLot }: LotsPageProps) {
  ...
  return <div className="space-y-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-[23px] font-semibold tracking-tight">Lotes</h1><p className="mt-1 text-[12px] text-muted">{lots.filter((lot) => lot.campaign === 2026).length} lotes en campaña 2026 · {lots.filter((lot) => lot.origin === 'Procedencia incompleta').length} con procedencia incompleta</p></div><button type="button" onClick={() => setShowForm(true)} className="flex min-h-10 items-center gap-1 rounded-xl bg-plum px-3.5 text-[12px] font-semibold text-white hover:bg-plum-dark"><Plus className="size-4" />Nuevo lote</button></div><div className="flex flex-wrap gap-2 rounded-2xl border border-border bg-[#fdfbfc] p-3 sm:p-4">{([['Campaña', campaign, setCampaign, ['Todas', '2026', '2025']], ['Categoría', category, setCategory, categoryOptions], ['Archivo', archive, setArchive, ['Activos', 'Archivados', 'Todos']]] as const).map(([label, value, setter, options]) => <label key={label} className="text-[11px] font-semibold text-muted">{label}<select value={value} onChange={(event) => setter(event.target.value)} className="ml-2 min-h-9 rounded-lg border border-border bg-white px-2 text-[11.5px] font-medium text-ink">{options.map((option) => <option key={option}>{option}</option>)}</select></label>)}</div>{loading ? <p className="p-6 text-center text-[12px] text-muted">Cargando lotes…</p> : filtered.length === 0 ? <div className="rounded-2xl border border-border bg-white p-8 text-center text-[13px] text-muted">No hay lotes con estos filtros.</div> : <div className="overflow-hidden rounded-2xl border border-border bg-white"><div className="hidden grid-cols-[1.35fr_.7fr_.7fr_.8fr_.85fr] gap-3 bg-[#fdfbfc] px-4 py-3 text-[10.5px] font-semibold text-muted sm:grid"><span>Lote</span><span>Categoría</span><span>Unidades</span><span>Volumen total</span><span>Estado</span></div>{filtered.map((lot) => { const locations = locationsFor(lot); const state = lot.archived ? 'Archivado' : lot.contentCodes.length === 0 ? 'Sin contenido' : lot.contentCodes.length > 1 ? 'Estados diversos' : locations[0] ? activeOccupation(locations[0])?.alcoholicState : 'Sin ubicación'; return <button key={lot.code} type="button" onClick={() => setSelected(lot)} className="grid w-full gap-2 border-t border-border px-4 py-3 text-left text-[11.5px] hover:bg-[#fdfbfc] sm:grid-cols-[1.35fr_.7fr_.7fr_.8fr_.85fr] sm:items-center sm:gap-3"><div><span className="font-mono text-[12.5px] font-semibold text-plum">{lot.code}</span><div className="mt-1 text-[11px] text-muted">{lot.origin} · {lot.variety}</div></div><span>{lot.category}</span><span className="text-muted">{lot.contentCodes.length ? `${lot.contentCodes.length} activa${lot.contentCodes.length === 1 ? '' : 's'}` : 'Ninguna'}</span><span className="font-mono">{formatLiters(volumeFor(lot))} L</span><span className={`w-fit rounded-full px-2 py-1 text-[10.5px] font-semibold ${lot.archived ? 'bg-[#efeff5] text-[#43435c]' : lot.contentCodes.length > 1 ? 'bg-plum-soft text-plum' : lot.contentCodes.length === 0 ? 'bg-[#f5e4da] text-[#7a4a22]' : 'bg-[#dceadf] text-[#1f5c3a]'}`}>{state}</span></button> })}</div>}
    ...
    {showForm && <LotForm deposits={deposits} existingLots={lots} defaultResponsible={profile?.displayName ?? ''} defaultResponsibleUsername={profile?.username ?? ''} onClose={() => setShowForm(false)} onCreated={(lot) => { setShowForm(false); setNotice(`${lot.code} creado${lot.contentCodes.length ? ' con entrada confirmada' : ' sin contenido registrado'}.`); load() }} />}
    {selected && <div className="fixed inset-0 z-30 flex justify-end bg-[#2e262a]/30" onMouseDown={() => setSelected(undefined)}><section role="dialog" aria-modal="true" aria-labelledby="lot-detail-title" onMouseDown={(event) => event.stopPropagation()} className="h-full w-full max-w-[420px] overflow-y-auto bg-[#fdfbfc] p-5 shadow-xl sm:p-6"><div className="flex items-start justify-between gap-3"><div><p className="m-0 font-mono text-[10px] text-muted">LOTE · {selected.campaign}</p><h2 id="lot-detail-title" className="mt-1 font-mono text-[20px]">{selected.code}</h2>{selected.archived && <span className="mt-1 inline-block rounded-full bg-[#efeff5] px-2 py-0.5 text-[10px] font-semibold text-[#43435c]">Archivado</span>}</div><button type="button" onClick={() => setSelected(undefined)} aria-label="Cerrar" className="rounded-lg p-2 hover:bg-plum-soft"><X className="size-4" /></button></div><dl className="mt-5 space-y-3 rounded-2xl border border-border bg-white p-4 text-[12px]">{[['Categoría', selected.category], ['Destino', selected.destination], ['Responsable', selected.responsible], ['Procedencia', selected.origin], ['Variedad', selected.variety], ['Fecha', selected.entryDate]].map(([label, value]) => <div key={label} className="flex justify-between gap-4"><dt className="text-muted">{label}</dt><dd className="m-0 text-right font-semibold">{value}</dd></div>)}</dl><h3 className="mt-5 text-[14px] font-semibold">Ubicaciones actuales</h3>{locationsFor(selected).length ? locationsFor(selected).map((deposit) => <button key={deposit.id} type="button" onClick={() => onOpenDeposit(deposit.code)} className="mt-2 flex w-full justify-between rounded-xl border border-border bg-white p-3 text-[12px] hover:bg-plum-soft"><span className="font-mono text-plum">{deposit.code}</span><span>{formatLiters(activeOccupation(deposit)?.volumeLiters ?? 0)} L <ArrowRight className="inline size-3" /></span></button>) : <p className="mt-2 text-[12px] text-muted">Sin contenido activo registrado en depósitos.</p>}<div className="mt-6 border-t border-border pt-4">{selected.archived ? <p className="text-[12px] text-muted">Este lote ya está archivado y no se muestra en el listado activo. Filtra por "Archivados" para verlo.</p> : archiving ? <div className="space-y-3 rounded-2xl border border-[#d6a8b3] bg-[#fdf5f7] p-3"><p className="text-[12px] font-semibold text-[#8e1f33]">Archivar lote {selected.code}</p><p className="text-[11.5px] text-copy">Vas a retirar este lote del listado activo. Quedará en el filtro «Archivados» junto con su motivo; los contenidos creados a partir de él no se modifican.</p><label className="block text-[12px] font-semibold text-copy">Motivo<textarea required rows={2} value={archiveReason} onChange={(event) => setArchiveReason(event.target.value)} placeholder="p. ej. Error al registrar las variedades" className="mt-1 w-full rounded-xl border border-border bg-white p-2 text-[12px] outline-none focus:border-[#b9899c]" /></label>{archiveError && <p role="alert" className="text-[12px] text-[#8e3b4a]">{archiveError}</p>}<div className="flex flex-wrap gap-2"><button type="button" disabled={archiveBusy || !archiveReason.trim()} onClick={async () => { setArchiveBusy(true); setArchiveError(''); try { await cellarApi.archiveLot(selected.code, archiveReason.trim()); setNotice(`Lote ${selected.code} archivado. Filtra por "Archivados" si quieres seguir viéndolo.`); setSelected(undefined); setArchiving(false); setArchiveReason(''); load() } catch (cause) { setArchiveError(cause instanceof Error ? cause.message : 'No se ha podido archivar el lote.'); setArchiveBusy(false) } }} className="min-h-10 rounded-xl bg-[#8e1f33] px-4 text-[12px] font-semibold text-white disabled:opacity-60">{archiveBusy ? 'Archivando…' : 'Confirmar archivado'}</button><button type="button" onClick={() => { setArchiving(false); setArchiveReason(''); setArchiveError('') }} className="min-h-10 rounded-xl border border-border px-4 text-[12px] font-semibold">Cancelar</button></div></div> : <button type="button" onClick={() => setArchiving(true)} className="min-h-9 rounded-xl border border-[#d6a8b3] px-3 text-[11.5px] font-semibold text-[#8e1f33] hover:bg-[#f7e0e6]">Archivar lote</button>}</div></section></div>}
  </div>
}
