...
import { ArrowDown, ArrowUp, ArrowUpDown, ChevronDown, LayoutGrid, List, Plus, SlidersHorizontal, X } from 'lucide-react'
import { DepositCard } from '../components/deposit-card'
import { ContentBadge, LocationBadge, LocationIcon, contentDotClass, locationKey, locationOrder } from '../components/deposit-badges'
...
type SortKey = 'code' | 'zone' | 'content' | 'fill' | 'phases' | 'status'
...
function compareBy(key: SortKey, a: Deposit, b: Deposit): number {
  ...
  switch (key) {
    ...
    case 'phases': return collator.compare(occA ? stateLabel(fermentationLabels, occA.alcoholicState) : '￿', occB ? stateLabel(fermentationLabels, occB.alcoholicState) : '￿') || collator.compare(occA ? stateLabel(fermentationLabels, occA.malolacticState) : '', occB ? stateLabel(fermentationLabels, occB.malolacticState) : '')
    case 'status': return rankOf(a) - rankOf(b)
  }
}

const sortLabels: Record<SortKey, string> = { code: 'Depósito', zone: 'Localización', content: 'Contenido', fill: 'Llenado', phases: 'Fases', status: 'Estado' }
...
export function DepositsPage({ search, onOpenDeposit }: DepositsPageProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.sort((a, b) => factor * compareBy(sort.key, a, b) || collator.compare(a.code, b.code))
  }, [deposits, search, zone, content, status, priority, material, refrigerated, sort])
  ...
  const activeFilters = [zone, content, status, priority, material, refrigerated].filter(Boolean).length
  ...
  const resetFilters = () => { setZone(ALL); setContent(ALL); setStatus(ALL); setPriority(ALL); setMaterial(ALL); setRefrigerated(ALL) }
  ...
  const stats = <>{deposits.length} recipientes · {deposits.filter((item) => item.status === 'occupied').length} ocupados · {deposits.filter((item) => item.priority === 'overdue').length} con control vencido</>
  return <div className="space-y-3 sm:space-y-4">
    {/* On mobile the section tab already says "Depósitos": only the figures and a compact "Nuevo". */}
    <div className="flex items-center justify-between gap-3 sm:items-end">
      <div className="min-w-0"><h1 className="hidden text-[23px] font-semibold tracking-tight sm:block">Depósitos</h1><p className="truncate text-[11.5px] text-muted sm:mt-1 sm:text-[12px]">{stats}</p></div>
      <div className="flex shrink-0 gap-2"><div className="hidden rounded-xl border border-border bg-white p-1 sm:flex"><button type="button" onClick={() => setView('table')} aria-label="Vista de tabla" aria-pressed={view === 'table'} className={`rounded-lg p-2 ${view === 'table' ? 'bg-plum-soft text-plum' : 'text-muted'}`}><List className="size-4" /></button><button type="button" onClick={() => setView('cards')} aria-label="Vista de tarjetas" aria-pressed={view === 'cards'} className={`rounded-lg p-2 ${view === 'cards' ? 'bg-plum-soft text-plum' : 'text-muted'}`}><LayoutGrid className="size-4" /></button></div><button type="button" onClick={() => setShowForm(true)} className="flex h-9 items-center gap-1 rounded-xl bg-plum px-3 text-[12px] font-semibold text-white hover:bg-plum-dark sm:h-10 sm:px-3.5"><Plus className="size-4" /><span className="sm:hidden">Nuevo</span><span className="hidden sm:inline">Nuevo depósito</span></button></div>
    </div>
    <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="ml-auto flex items-center gap-3">
        {!loading && <p className="hidden text-[11.5px] text-muted sm:block" aria-live="polite">{filtered.length === deposits.length ? `${filtered.length} depósitos` : `${filtered.length} de ${deposits.length}`}</p>}
