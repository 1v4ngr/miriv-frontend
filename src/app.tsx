...
import { BlendPage } from './features/blend/pages/blend-page'
...
import { AccountPage, AuditPage } from './features/system/pages/system-pages'
...
export function App() {
  ...
  if (route.startsWith('contents/')) return <CellarShell search={cellarSearch} onSearchChange={setCellarSearch} activeSubsection="Contenidos" onNavigate={navigate}><ContentDetailPage code={decodeURIComponent(route.slice('contents/'.length).split('?')[0])} openToWine={route.endsWith('?to-wine')} onBack={() => navigate('deposits')} onOpenDeposit={openDeposit} onOpenLot={openLot} onOpenLaboratory={openLaboratory} onOpenTracking={(code) => navigate(`tracking/${encodeURIComponent(code)}`)} /></CellarShell>
  if (route === 'dashboard' || route.startsWith('dashboard/')) return <CellarShell search={cellarSearch} onSearchChange={setCellarSearch} activeSubsection="Seguimiento" onNavigate={navigate}><DashboardPage dashboardId={route.startsWith('dashboard/') ? decodeURIComponent(route.slice('dashboard/'.length)) : undefined} onNavigate={navigate} /></CellarShell>
  if (route === 'blend' || route.startsWith('blend/') || route.startsWith('blend?')) return <CellarShell search={cellarSearch} onSearchChange={setCellarSearch} activeSubsection="Seguimiento" onNavigate={navigate}><BlendPage key={route.split('?')[0]} route={route} onNavigate={navigate} /></CellarShell>
  if (route === 'tracking') return <CellarShell search={cellarSearch} onSearchChange={setCellarSearch} activeSubsection="Seguimiento" onNavigate={navigate}><TrackingListPage onOpenCurves={(code) => navigate(`tracking/${encodeURIComponent(code)}`)} onCompare={(codes, parameters) => navigate(`tracking/compare?c=${codes.join(',')}&p=${parameters.join(',')}`)} onSimulate={(codes) => navigate(`blend?c=${codes.join(',')}`)} /></CellarShell>
  ...
  if (route === 'reports' || route.startsWith('reports?')) { const deposit = new URLSearchParams(route.split('?')[1] ?? '').get('deposit') ?? undefined; return <CellarShell search={cellarSearch} onSearchChange={setCellarSearch} activeSubsection="Informes" onNavigate={navigate}><ReportsPage key={route} initialDeposit={deposit} /></CellarShell> }
